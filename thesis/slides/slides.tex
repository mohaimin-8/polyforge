...
\begin{frame}{Thesis statement}
  ...
  \end{quote}
  \vfill
  Tested under \textbf{13 pre-registered protocols} (pushed before runs),
...
\begin{frame}{Methodology: the part built to be attacked}
  \begin{itemize}
    \item 13 pre-registrations committed \textbf{and pushed before any run};
...
\begin{frame}{Validity: what bounds the simulator gap}
  \begin{itemize}
    ...
    \item \textbf{Real data carries the claims}: BurstGPT $+$ Azure demand,
          LMSYS prompts.
    \item \textbf{Live check executed, ordinal-only}: both arms ran on a
          real kind cluster (12/12 valid); the pre-frozen protocol recorded a
          \alert{DISAGREE} --- live \emph{parity} where the sim separates the
          arms (cache/tier knobs inert live by construction). Published as
          measured; no claim rested on live-ranking transfer.
  \end{itemize}
  \vfill
...
\begin{frame}{What remains, and closing}
  \textbf{Done since the proposal:} the live JCAC ordinal slice ran
  (12/12, DISAGREE published); tier-price and empirical-cache-curve reruns
  (both PASS 5/5); sim chaos (a dead planner still beats a healthy HPA);
  planner cells to \textbf{1024 tenants}; the isolation attack executed
  over the wire.
  \vfill
  \textbf{Remaining (user-gated, GPU host):} the \alert{three-knob live
  plane} --- the joint controller with cache \emph{and} tier as live levers,
  not just replicas (pre-registered as the Wave 4 live plane); plus live
  chaos and a live p99 number.
  \vfill
  \begin{quote}
