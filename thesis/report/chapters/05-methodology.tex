...
public repository \emph{before its first run}. Seven pre-registrations
exist: \evfile{PREREG\_V2.md}, \evfile{PREREG\_V3.md},
\evfile{PREREG\_TRACE.md}, \evfile{PREREG\_TRACE2.md},
\evfile{PREREG\_VTC.md}, plus the v1 gate and the Phase-4 acceptance frozen
in the roadmap documents. Protocol changes forced by feasibility (e.g.\ the
LMSYS similarity matrix that would not fit in 40\,TB) were committed as
\emph{declared pre-run amendments} before the gated data was downloaded ---
the amendment chain is the pre-registration's honest continuation, visible
in git history. Closed campaigns are immutable: stopping rules forbid
re-running, widening, or post-hoc tuning, and campaign result files are
never edited after their run.
