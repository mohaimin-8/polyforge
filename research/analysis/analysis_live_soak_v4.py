...
EV = REPO / "eval" / "results" / "live_soak_evidence"
...
def score_skh1(fine: list[dict], fault_list) -> tuple[list[dict], bool | None]:
    rows, verdict = [], True
...
def score_skh2() -> tuple[list[dict], bool | None]:
    ...
    for r in csv.DictReader(path.read_text(encoding="utf-8").splitlines()):
        ...
        verdict = verdict and ok
        rows.append({"label": r["label"], "healthy": healthy,
                     "degraded": degraded, "floor": floor, "ok": ok})
    return rows, (verdict if rows else None)
...
def build() -> str:
    ...
    h1_rows, h1 = score_skh1(fine.get("buckets", []), fault_list)
    h2_rows, h2 = score_skh2()
    ...
    for r in h1_rows:
        if not r["scoreable"]:
            w(f"| {r['name']} | — | — | — | — | not scoreable (run ended) |")
            continue
