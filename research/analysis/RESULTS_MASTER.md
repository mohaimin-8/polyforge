...
`docs/WAVE3_LIVE_RUNBOOK.md`. *Update (audit 2026-09-26): both have since run (`RESULTS_LIVE_CHAOS_P99.md`); that sitting's planner read zero demand, so its LC-H1 could not fail (`ERRATUM_ZERO_DEMAND_PLANNER.md`).*
...
| Fallback under failure | **jcac with a 1-min-dead planner beats a healthy HPA (CH-H1, dz −1.69); a freeze costs jcac *less* than HPA (`RESULTS_CHAOS_SIM.md`)** | any live-chaos claim | the chaos campaign is sim-substrate (controllers blind, engine-injected); the live chaos demonstration has since run (`RESULTS_LIVE_CHAOS_P99.md`), but with a planner that saw zero demand, so LC-H1 could not fail (`ERRATUM_ZERO_DEMAND_PLANNER.md`; audit 2026-09-26) |
...
| Cost/J on the second real trace | published −42%/window, d_z −1.64…−1.85, p≤5.3e-22, n=72, PASS-with-disclosure (`RESULTS_TRACE_AZURE.md`); **re-scored fairly (session 37) the advantage survives at −7.1%, PASS, p≤9.5e-05, pervasive across 69.4% of windows (`RESULTS_TRACE_PARITY.md`) — cite the adjudicated number** **Superseded (audit 2026-09-26): BP-H1 FAILs on Azure (−3.3%/−1.3%, p=0.379/0.91, `RESULTS_BUDGET_PARITY.md`), so −7.1% is withdrawn with the other comparative cost claims (row "Cost"); do not cite it.** | the −42% published figure alone, uncaveated; any pooling with BurstGPT numbers | `RESULTS_TRACE_AZURE.md`'s comparator carried the 1.4581× LRU charge / 128 MB cache pin (WP1); shrank but survived fair accounting, then failed budget parity (BP-H1) — no longer a surviving cost claim (audit 2026-09-26). Independent trace, different window length (3 h), pseudo-tenantized — own substrate row, never pooled with BurstGPT (ground rule 4) |
...
1. Every campaign's protocol was committed and pushed **before** its first run —
   **except two, disclosed** (audit 2026-09-26): EVICTION_PARITY and ORDER_PERMUTATION
   were first committed after their campaigns' first runs. `scripts/check_prereg_timing.py`
   now judges every campaign against its protocol's first commit in CI (48 judged; runs
   under two minutes after the commit are flagged "tight"). The count below is the
   state at session 44; **51 protocols** exist as of 2026-09-26, and
   `OSF_REGISTRATION.md` covers the first 29 only.
...
5. Sim latency metrics are p95, not p99 — the sim is a p95 estimator by construction
   ...
   p99 reading (`PREREG_LIVE_CHAOS_P99.md`, since run) is never back-fitted into a sim
   table.
