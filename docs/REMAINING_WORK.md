...
| D9 | no envtest; fake clients hide conflicts | **WRITTEN + CI-WIRED, local execution pending** — `envtest_conflict_test.go` (build tag `envtest`) covers the three things fake clients cannot reproduce: CEL rejection of inverted bands, an audit record surviving a real status-subresource 409, and pinned knobs holding through an apiserver round-trip. Runs in CI on every push. `go test ./...` stays green without the binaries. **Do not describe this as passing until it has been executed** — it compiles and vets under the tag; the local binary download was still in progress at end of session 35 |
