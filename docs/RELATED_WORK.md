...
|---|---|---|
| Sim substrate vs their live GPU fleets | Medium (was High) | **Phase 6 DONE both halves** (`CALIBRATION.md`, `TIER_BENCH.md`): g(ρ)=1/(1−ρ) holds to first order (a=0.86), conservative against lean operation; real per-tier GPU latency table measured, ordering confirmed. **Phase 7 pipeline VERIFIED live (session 16d):** first cluster-backend run valid on a real kind cluster — real HPA, real load, eval-export metrics matching the load's construction exactly. Remaining: operator/planner chart wiring, then the PolyForge-vs-HPA ordinal figure |
...
| Real-LMSYS cache headline | ~~Medium~~ | **CLOSED (session 16c):** measured on the gated dataset — 29.6% @ 0.85 / 48.8% @ 0.70, +93% vs 10%-fixed, $-weighted savings attached; pre-run feasibility amendment committed before download (6f0c3d4) |
