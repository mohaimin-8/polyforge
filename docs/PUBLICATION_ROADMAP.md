...
| **WP14 live CRUD soak** | **BLOCKED on WP8a's actuation half** — prereg frozen (`PREREG_LIVE_SOAK.md`), driver spec written, but the live path fails at minute 2 | `REMAINING_WORK.md` §WP8a actuation half: encoding bug FIXED; `POST /v1/tenants` hangs at 60 s in both storage modes, hypothesis = 16 control-plane replicas on a 2-node `small` cluster. Do NOT start a 12–24 h soak until the dry-run passes |
