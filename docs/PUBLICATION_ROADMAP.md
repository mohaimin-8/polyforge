...
| **WP8b B1 scored** | **RAN (session 41, 2026-08-30) — SUBSTRATE INADEQUATE, WL-H1 VOID.** The GPU gate is no longer the blocker; the knobs are. | `RESULTS_WAVE4_LIVE_PLANE.md`. Ran on the free split-host route (Kaggle P100 serving Qwen2.5-0.5B + 3B behind a cloudflared tunnel, `docs/WAVE4_FREE_ROUTE.md`). The route was fine — `tunnel_preflight` PASSED with a **660.9 ms** tier gap vs a 341.8 ms threshold and 472 ms of premium-SLO headroom. **WL-H2's gate then returned SUBSTRATE INADEQUATE**: the tier pin is **asymmetric** — pinning `small` serves `['small']`, pinning `mid` serves `['mid','small']`, so `routing_moved` is false and `latency_moved` false *because of* the leak (266.8 ms vs the 322.6 ms required). The cache knob is under-powered, not inert (delta 0.29 vs 0.5 margin). **No margin was lowered**, per the prereg. Getting there required fixing **four** harness defects in code that had never executed: `NameError: EVAL_DIR`, a `UnicodeEncodeError` printing the gate's own verdict, `knob_preflight.json` self-deleting with its temp workdir, and tiers ordered **alphabetically** rather than by capability — the last was *manufacturing* an `ordering_correct: false`. Next per §7: fix the gateway's tier pin, re-sit under a NEW prereg. |
